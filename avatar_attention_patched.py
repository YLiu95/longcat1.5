...
class SingleStreamAttention(nn.Module):
    ...
    def _process_cross_attn(self, x, cond, frames_num=None, x_ref_attn_map=None, human_num=None):
        ...
        if self.enable_flashattn3:
            ...
            orig_dtype = q.dtype
            q_f16 = q.to(torch.float16)
            ek_f16 = encoder_k.to(torch.float16)
            ev_f16 = encoder_v.to(torch.float16)
            with sdpa_kernel(SDPBackend.EFFICIENT_ATTENTION):
                x = torch.nn.functional.scaled_dot_product_attention(q_f16, ek_f16, ev_f16, scale=self.scale)
            x = x.to(orig_dtype)
