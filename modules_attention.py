...
class MultiHeadCrossAttention(nn.Module):
    ...
    def _process_cross_attn(self, x, cond, kv_seqlen):
        ...
        if self.enable_flashattn3:
            ...
            # SDPA fallback for variable-length cross-attention
            # q: [1, B*N, H, D], k: [1, sum_kv, H, D] - need to split by batch
            q_split = q[0].split(N, dim=0)  # list of [N, H, D]
            k_split, v_split = [], []
            offset = 0
            for sl in kv_seqlen:
                k_split.append(k[0, offset:offset+sl])  # [sl, H, D]
                v_split.append(v[0, offset:offset+sl])
                offset += sl
            outputs = []
