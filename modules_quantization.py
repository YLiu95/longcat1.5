...
def load_quantized_dit(checkpoint_dir: str, subfolder: str = "base_model_int8", **kwargs):
    """Load a quantized DiT model with minimal RAM usage.

    Uses meta device initialization to avoid allocating real tensors during
    model construction, then loads weights shard-by-shard.
    """
    ...
    # Instantiate model on meta device (zero RAM cost)
    with torch.device('meta'):
        model = LongCatVideoAvatarTransformer3DModel(**config)

    # Replace Linear layers with QuantizedLinear (also on meta)
    skip_patterns = DEFAULT_SKIP_PATTERNS
    modules_to_replace = {}
    for name, module in model.named_modules():
        if isinstance(module, nn.Linear):
    ...
    # Load shards one at a time with assign=True to materialize from meta
